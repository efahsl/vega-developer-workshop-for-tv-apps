...
const ThumbnailItem = React.memo(
  ({item, onPress, onFocus}: ThumbnailItemProps) => {
...
const ContentRow = React.memo(({
  ...
  const renderItem = useCallback(
    ...
    [onItemPress, onItemFocus],
  );

  const keyExtractor = useCallback(
    (item: MovieItem, index: number) => `${index}-${item.id}`,
    [],
  );
...
// Enable WhyDidYouRender tracking
ThumbnailItem.whyDidYouRender = true;
ContentRow.whyDidYouRender = true;
...
export const HomeScreen = ({navigation}: HomeScreenProps) => {
  ...
  const handleItemFocus = useCallback((item: MovieItem) => {
    // setBackgroundImage(item.images.poster_16x9);
    setBackgroundImage(item.images.thumbnail_450x253);
  }, []);

  // Group movies by category - memoized to prevent recalculation
  ...
  // Get trending movies - memoized to prevent recalculation
  const trendingMovies = useMemo(() => {
    return movies.filter((movie) => movie.trending === true);
  }, [movies]);

  const categories = useMemo(() => Object.keys(moviesByCategory), [moviesByCategory]);
