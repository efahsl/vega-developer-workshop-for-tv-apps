...
We will use the following prompt, make sure you read it all. The benefit of using prompts is that we can use natural language to describe the experience we want to build, while still specifying some technical direction and details.
...
Now let's build the actual DetailsScreen.
...
### Add Actions/Buttons to the Screen

Now let's add some buttons to the screen so we can do something - we'll add a "Play" button which will bring us to a new Video Playback Screen (to be built next) and an "Add to watchlist" button that is a placeholder and won't be built in this workshop (though you can try doing it yourself if you like!)
...
OK now we're going to add a video player. Note that with Vega we are using W3C Media APIs, of which you can read up more [HERE](https://developer.amazon.com/docs/vega/0.21/media-playback.html). Our Vega media player has two options:
...
Let's make our Home Screen a little more realistic by using multiple rows and categories of content.
