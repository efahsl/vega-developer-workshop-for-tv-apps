...
**Download our Vega MCP Server tarball** *(in the future we will improve this experience, but for now it requires a manual download)* - Download [vega-devtools-mcp.tgz](https://quip-amazon.com/-/blob/XHR9AAz7WE2/FDYqA9QKC-sT6IAT-5ibZg?name=vega-devtools-mcp.tgz&s=ROPyAzh306H2). Note down the download path
...
Save this path to your clipboard or another text file, you will need for the next step, which is the update the relevant
...
| 2 | Github Copilot | ~/.config/mcp-config.json<br><project-root>/.vscode/mcp.json |
| 3 | Claude Code CLI | ~/.claude.json<br>/Library/Application Support/ClaudeCode/managed-mcp.json |
...
Skip npm start skip in the AI agent, by click skip and start it manually in a terminal window
